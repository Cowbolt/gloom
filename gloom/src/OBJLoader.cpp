...
// This function assumes a mesh with rectangular sides (pairs of triangles), and assigns each side random colors.
// It also assumes vertices have been duplicated, which is done by the loadWavefront function.

void colorFaces(Mesh &mesh) {
	int sides = mesh.faceCount() / 2;

	// Allocate capacity
	mesh.colors.resize(mesh.vertices.size(), 0);
	...
	for(int side = 0; side < sides; side++) {
		...
		mesh.colors.at(side * 6 + 0) = randomColour;
		mesh.colors.at(side * 6 + 1) = randomColour;
		mesh.colors.at(side * 6 + 2) = randomColour;
		mesh.colors.at(side * 6 + 3) = randomColour;
		mesh.colors.at(side * 6 + 4) = randomColour;
		mesh.colors.at(side * 6 + 5) = randomColour;
	}
}
...
MinecraftCharacter loadMinecraftCharacterModel(std::string const srcFile) {
	...
	for(Mesh mesh : fileContents) {
	    // Applying some color to the different parts
